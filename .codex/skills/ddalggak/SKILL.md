...
- **Wiki Context First (all subcommands, mandatory)**: every subcommand must run `references/wiki-context-preflight.md` before its work and include a Wiki Context Manifest before its completion signal; when no relevant wiki exists or search fails, record `Wiki: none-found` and proceed with normal LLM behavior (wiki strengthens, never blocks). Cite wiki paths for wiki-derived claims.
