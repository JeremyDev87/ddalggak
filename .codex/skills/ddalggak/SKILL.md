...
- **Runtime contract language**: use task scope contract, Context Assembly Manifest, Resume Snapshot, and Control-flow ownership to make conductor/reviewer-owned boundaries explicit. This increases clarity, not hidden worker autonomy.
- **Task Scope Contract**: every plan, worker brief, review packet, and fix brief must separate the **tool capability boundary** (what the agent could technically do) from the **task scope contract** (what this task authorizes). The contract must name Goal, Authorized files, Forbidden files/actions, Allowed side effects, Escalation-required actions, Validation commands, and Completion evidence. Any out-of-scope action or out-of-scope diff is an authorization / scope-expansion failure, not a capability failure; reviewers classify unauthorized files, unrelated cleanup/refactor, config/credential/migration changes, destructive actions, external API writes, production data touch, force-push, or branch mutation as High or Critical candidates.
- **Context Assembly Manifest**: plan/start/review packets must name the context sources used by the worker or reviewer: issue body and comments, repo conventions, loaded skills/references, applicable gates, skipped gates and reasons, assumptions, and known blockers. The conductor owns context window assembly; workers do not treat incidental context as the source of truth.
- **Resume Snapshot**: before idle waits, CI waits, review/fix loops, wave transitions, and handoff rescue, record the current phase, issue/branch/PR, changed files, validation evidence, blocking gaps, waiting-on state, next gate, and exact next command or expected completion signal.
- **Control-flow ownership**: approval gates, retries, destructive actions, external side effects, force-push, production data touch, and verification completion are conductor/reviewer-owned deterministic gates. Do not bury them inside an autonomous worker loop.
- **Small focused workers, explicit orchestration**: small focused agents reduce worker responsibility and context size; they do not reduce safe parallelism. The conductor owns issue decomposition, branch/PR state, review/fix gates, and wave transitions.
- **Conflict-fallback planning**: every parallel plan must name owned files, must-not-touch files, why each issue can have its own PR, lane-specific evidence/validation, and conflict gates. Shared files, shared contracts, runtime flips, or a task scope contract that must transition atomically make affected issues serial commits in one fallback PR, not the default for independent issues.
...
- **Wiki growth triage**: before promoting wiki-derived knowledge into ddalggak, read `references/wiki-growth-triage.md` and choose the smallest durable surface: immediate guardrail, reference/template only, verifier/script candidate, GitHub issue candidate, or defer/reject. Preserve Issue-PRs by default, and allow conflict fallback only for concrete hard conflicts.
...
- Hard blockers include the same existing file, same generated artifact, shared registry or barrel, shared schema, unpublished code dependency, gitignored or local-only path, repo-external file, atomic transition of the same delivery or output contract, and atomic transition of the same task scope contract.
...
8. Write a brief per lane. Each brief must include:
   ...
   - Task Scope Contract with Goal, Authorized files, Forbidden files/actions, Allowed side effects, Escalation-required actions, Validation commands, and Completion evidence; explicitly state that the tool capability boundary is broader than the task scope contract and that out-of-scope diff is a scope-expansion failure;
...
2. For each PR, collect a review packet before spawning review:
   ...
   - issue body and comments, Quality Lens Router Output, Evidence Contract, Task Scope Contract, validation already run, skipped checks, constraints, Review Rubric, AI Code Quality Gate checklist, and commit-lane order context.
   - Diff Footprint / Scope Expansion Review: compare the changed files, side effects, config/credential/migration/branch operations, and cleanup/refactor footprint against the task scope contract; treat out-of-scope diff and scope-expansion failure as High or Critical candidates.
...
- **Diff Footprint / Scope Expansion Review**: Does every changed file and side effect fit the Task Scope Contract? Are tool capability boundary and task scope contract separated? Flag out-of-scope diff, unrelated cleanup/refactor, config/credential/migration changes, destructive actions, external API writes, production data touch, force-push, or branch mutation as scope-expansion failure candidates.
