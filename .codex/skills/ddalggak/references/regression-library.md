...
| Skill payload parity | A packaged skill reference, template, or CLI-dispatched doc changes. | The shared Claude/Hermes root and Codex projection stay mirrored where required, package dry-run includes the files, and verifier/smoke checks cover exposed anchors. | Only one physical payload tree is changed, package inclusion is not checked, or stale command-surface wording remains. |
