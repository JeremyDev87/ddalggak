...
- No stacked PRs by default: ddalggak should use one base branch, one integration branch, one PR, and separate commits for independent lanes unless the user explicitly asks for stacked PRs.
- `plan`, `issue`, and `start` should prove lane independence before claiming parallelism. The single-PR commit-lane matrix must include owned files, must-not-touch files, why each lane is independent, lane-specific evidence/validation, and the integration gate. Shared files, shared contracts, or runtime flips become serial commits in the same PR, not parallel lane PRs.
