...
- **Promotion target**: issue number, intended branch/PR, and whether the candidate affects the shared Claude/Hermes root, the Codex projection, or both physical payloads.
...
5. **Mirror parity gate**: when packaged skill surfaces are mirrored, update and verify the shared Claude/Hermes root and Codex projection or explain why only one physical root is in scope.
