...
Execution contract index: live PR state, diff/files/checks, linked issue, current head SHA, wiki-context preflight, Quality Lens Router, Evidence Contract, Simplicity / Deletability, Core Invariants, conditional frontend/vercel/regression gates, blocker triage, inline line-anchored finding comments in one COMMENT-event batch, top-level comment with SHA, validation, conclusion.
